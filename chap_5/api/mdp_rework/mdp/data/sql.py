...
def flush_policy(engine, rows: list):
    if not rows:
        return
    data = [{"s": json.dumps(list(r["s"])), "value": json.dumps(list(r["value"]))} for r in rows]
    query = text("""
        INSERT INTO policy (s, value)
        VALUES (:s, :value) ON CONFLICT(s) DO
...
def flush_value_states(engine, rows: list):
    if not rows:
        return
    data = [{"s": json.dumps(list(r["s"])), "value": r["value"]} for r in rows]
    query = text("""
        INSERT INTO value_state (s, value)
        VALUES (:s, :value) ON CONFLICT(s) DO
        UPDATE SET value = excluded.value
    """)
    with engine.begin() as conn:
...
def get_by_batch_policy(engine, candidates_set: set) -> dict:
    if not candidates_set:
        return {}
    candidates_list = [json.dumps(list(c)) for c in candidates_set]
    ...
    for s, value in rows:
        result[tuple(json.loads(s))] = json.loads(value)
    return result
...
def get_by_batch_states(engine, candidates_set: set) -> dict:
    ...
    with engine.connect() as conn:
        rows = conn.execute(query, {"candidates": candidates_list}).fetchall()
    for s, value in rows:
        result[tuple(json.loads(s))] = value
    return result